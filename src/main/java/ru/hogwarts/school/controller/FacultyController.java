...
public class FacultyController {
    ...
    public ResponseEntity createFaculty(@RequestBody Faculty faculty) { //Для записи факультетов по телу через свагер(постман)
        Faculty faculty1 = facultyService.createFaculty(faculty);
        return ResponseEntity.ok(faculty1);
    }

    @GetMapping("{id}") // GET http://localhost:8090/faculty/1
    public ResponseEntity findFaculty(@PathVariable Long id) { //Для получения факультета из Мапы по индексу через свагер(постман)
    ...
    public ResponseEntity editFaculty(@RequestBody Faculty faculty) { //Для редактирования факультетов в Мапе через свагер(постман)
    ...
    public ResponseEntity<Faculty> deleteFaculty(@PathVariable Long id) { //Для удаления факультета по id из Мапы через Свагер
        if (facultyService.deleteFaculty(id) == null) {
            return ResponseEntity.status(405).build(); //Если факультета с этим Id нет, то выскочит 405. Вариант 3
        }
        return ResponseEntity.ok(facultyService.deleteFaculty(id)); //При удалении студента по выбранному Id по умолчанию пропишется 404.
    }
